...
- [x] A `Diagram` inspector view on every register: the AHRM-style bit diagram as a live figure that both *reflects* and, **where the model says it is safe, writes** — sixteen bit cells grouped by named field with the mnemonic underneath, decoded values for multi-bit fields (BPU, VPOS/HPOS, R/G/B…), COLORxx with a live RGB swatch, and field-less registers showing their raw 16 bits. Writability is gated by a **write-safety policy on the model** (`GT4AmigaChipRegister>>writeSafetyOf:`, the single source of truth the Control Panel now shares), so the same figure can sit on every register without letting casual play brick the machine. Each bit carries a verdict: `#writable` (a latching bit safe to toggle — DMACON's channels, the non-critical INTENA/ADKCON bits — *clickable*, the click enqueues the write on the element's worker); `#transient` (writable but the Workbench copper rewrites the register every frame so it wouldn't persist — COLORxx, BPLCONx, DDF/DIW, modulos — amber border, not clickable); `#linkCritical` (a write cuts the monitor's own link — INTEN/RBF/TBE/PORTS, ADKCON UARTBRK — red border, locked); `#systemCritical` (a write holds a task — all of INTREQ, since a write injects a live interrupt, plus INTENA VERTB/EXTER — red border, locked); `#readOnly` (a read-mode register or a hardware-set indicator like BBUSY/BZERO); `#protocol` (the SETCLR command bit). Writes go through the same off-UI-thread queue+worker+readback contract as the Control Panel. This replaced a first cut that made *every* SET/CLR flag clickable with no policy — which reached INTREQ/ADKCON and the OS-critical INTENA bits and could hold a task ("Software error - task held") on a live poke; the policy is that regression's real fix. Hardening also folded in: a generation stamp retires a polling loop left over from a quick detach/re-attach (inspector tabs, exporters) without terminating it mid-`sendFrame` — terminating a worker inside a frame exchange leaves the client's I/O mutex owned by a dead process and desyncs the stream; let the retry ladder self-heal, or on a wedged link reboot FS-UAE for a clean monitor. **Verified live (2026-07-17)**: AUD0EN toggled through the diagram's click path with hardware readback (`$03F0`→`$03F1`→`$03F0`); INTENA renders INTEN/RBF/TBE and VERTB/EXTER with locked red borders, INTREQ all-red, BPLCON0's fields amber (transient, BPU=2/HIRES decoded from the copper-derived `$A200`); zero orphan workers after teardown.

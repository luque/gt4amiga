...
- [x] A `Diagram` inspector view on every register: the AHRM-style bit diagram as a live, **read-only** figure — sixteen bit cells grouped by named field with the mnemonic underneath, decoded values for multi-bit fields (BPU, VPOS/HPOS, R/G/B…), LED-styled read-only indicators (BBUSY/BZERO) and a paled SET/CLR protocol bit, COLORxx with a live RGB swatch, and field-less registers showing their raw 16 bits. It only *reflects*: toggling bits stays in the Control Panel, which exposes a vetted, contained surface (DMACON and INTENA, with the link-critical bits locked). A first cut made every SET/CLR flag on every register clickable, and that reached registers the panel deliberately never exposed — INTREQ (where a write injects a live hardware interrupt) and ADKCON (UARTBRK breaks the very serial line the monitor speaks over) — so casual play could poke the interrupt/DMA machinery under a running Workbench and hold a task ("Software error - task held"); making the diagram view-only removes that vector entirely. Hardening that also fell out: in all three live-element workers (control panel, DMA timeline, diagram) a quick detach/re-attach — inspector tab switches, exporters — could re-arm the shared `running` flag and leave *two* polling loops per element, piling up until the serial link starved; a generation stamp now retires the stale loop without terminating it mid-`sendFrame` (terminating a worker inside a frame exchange leaves the client's I/O mutex owned by a dead process and desyncs the stream — let the retry ladder self-heal instead). **Verified live (2026-07-17)**: DMACON/INTENA/COLOR00 render decoded and view-only (no click handlers in the subtree), the RGB swatch tracks COLOR00's $05A, and zero orphan workers after export-induced re-attach.

...
- [ ] One generic "live hardware" snippet type (evaluates to a model object, embeds its refreshing inspector) plus **one background observer process per page** that polls the addresses visible views need and publishes announcements — no per-view polling loops, no serial I/O on the UI thread. (Directly targets the concurrent-reader link-jam failure mode.)

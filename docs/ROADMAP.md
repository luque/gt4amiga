...
- [x] Replace the per-view polling loops with one shared observer. Directly targets the concurrent-reader link-jam failure mode. A **single serial gateway** owned by the catalog (`GT4AmigaChipRegisters>>observer`, a `GT4AmigaHardwareObserver`) is the only process that reads the machine; views declare what they need (`watch:for:` / `unwatch:`) and repaint from cache on the announcement; both page and non-page views feed the same gateway, so nothing ever contends for the socket.
  - [x] The shared serial gateway (`GT4AmigaHardwareObserver`): de-duplicated union reads, serialized writes, one background loop, generation-stamped, idles when nothing is watched. **Verified live (2026-07-18)**: the migrated Diagram subscribes/toggles/detaches cleanly (`$03F0`→`$03F1`→`$03F0`), and views over shared registers run under 1 reader process with each register read once, zero orphans after teardown.
  - [x] Migrate all views (register Diagram, Control Panel, DMA timeline, block diagram) off their own workers onto the gateway. **Verified live (2026-07-18)**: all four open at once = 4 subscribers over 5 distinct registers under 1 reader process (DMACON read once though four views show it), values fresh, a Control Panel switch write round-trips through the gateway (`$03F0`→`$03F1`→`$03F0`), and teardown leaves zero observer and zero legacy worker processes.
  - [x] Leak backstop: the gateway holds subscribers weakly (`WeakIdentityKeyDictionary`) **and** views subscribe to the announcer weakly (`announcer weak when:send:to:`) — the only two references that could pin a view. So a view ever dropped without its explicit unwatch (a virtualized page snippet whose removal event never fired) is garbage-collected: both references fall away and the loop idles. **Verified live (2026-07-18)**: a fully-subscribed view dropped *without* teardown drops to 0 subscribers, 0 announcer subscriptions and a stopped loop after GC, while normal attach/toggle/detach and multi-view dedup stay intact.
  - [ ] *(deferred — assessed unnecessary)* A page-level observer: the gateway already gives single-reader dedup, synchronized repaints (one sweep → one announce → all views repaint the same snapshot) and clean lifecycle, so multiple live figures sit on one page with no contention. Page controls (pause/resume/step/cadence) can be added to the gateway *if* a book page ever needs them; the "generic live-hardware snippet type" appears already covered by GT's native "a Pharo snippet returns a model → pick a gtView" — to be confirmed when authoring the next page.

...
- [ ] Un tipo de snippet "hardware vivo" genérico (evalúa a un objeto de modelo, embebe su inspector que se refresca) más **un proceso observador en background por página** que sondea las direcciones que las vistas visibles necesitan y publica anuncios — sin bucles de sondeo por vista, sin I/O serie en el hilo de UI. (Ataca directamente el modo de fallo de atasco del enlace por lectores concurrentes.)
